...
void Roomba_Init()
{
	...
	uart_init(UART_19200, CH_1);

	// start the Roomba's SCI
	uart_putchar(START, CH_1);
	_delay_ms(20);
	...
	uart_putchar(BAUD, CH_1);
	uart_putchar(ROOMBA_38400BPS, CH_1);
	_delay_ms(100);

	// change the AVR's UART clock to the new baud rate.
	uart_init(UART_38400, CH_1);

	// put the Roomba into safe mode.
	uart_putchar(CONTROL, CH_1);
	_delay_ms(20);
...
void Roomba_UpdateSensorPacket(ROOMBA_SENSOR_GROUP group, roomba_sensor_data_t* sensor_packet)
{
	...
	uart_putchar(SENSORS, CH_1);
	uart_putchar(group, CH_1);
	switch(group)
	{
	case EXTERNAL:
		// environment sensors
		while (uart_bytes_received(CH_1) != 10);
		sensor_packet->bumps_wheeldrops = uart_get_byte(0, CH_1);
		sensor_packet->wall = uart_get_byte(1, CH_1);
		sensor_packet->cliff_left = uart_get_byte(2, CH_1);
		sensor_packet->cliff_front_left = uart_get_byte(3, CH_1);
		sensor_packet->cliff_front_right = uart_get_byte(4, CH_1);
		sensor_packet->cliff_right = uart_get_byte(5, CH_1);
		sensor_packet->virtual_wall = uart_get_byte(6, CH_1);
		sensor_packet->motor_overcurrents = uart_get_byte(7, CH_1);
		sensor_packet->dirt_left = uart_get_byte(8, CH_1);
		sensor_packet->dirt_right = uart_get_byte(9, CH_1);
		break;
	case CHASSIS:
		// chassis sensors
		while (uart_bytes_received(CH_1) != 6);
		sensor_packet->remote_opcode = uart_get_byte(0, CH_1);
		sensor_packet->buttons = uart_get_byte(1, CH_1);
		sensor_packet->distance.bytes.high_byte = uart_get_byte(2, CH_1);
		sensor_packet->distance.bytes.low_byte = uart_get_byte(3, CH_1);
		sensor_packet->angle.bytes.high_byte = uart_get_byte(4, CH_1);
		sensor_packet->angle.bytes.low_byte = uart_get_byte(5, CH_1);
		break;
	case INTERNAL:
		// internal sensors
		while (uart_bytes_received(CH_1) != 10);
		sensor_packet->charging_state = uart_get_byte(0, CH_1);
		sensor_packet->voltage.bytes.high_byte = uart_get_byte(1, CH_1);
		sensor_packet->voltage.bytes.low_byte = uart_get_byte(2, CH_1);
		sensor_packet->current.bytes.high_byte = uart_get_byte(3, CH_1);
		sensor_packet->current.bytes.low_byte = uart_get_byte(4, CH_1);
		sensor_packet->temperature = uart_get_byte(5, CH_1);
		sensor_packet->charge.bytes.high_byte = uart_get_byte(6, CH_1);
		sensor_packet->charge.bytes.low_byte = uart_get_byte(7, CH_1);
		sensor_packet->capacity.bytes.high_byte = uart_get_byte(8, CH_1);//);
		sensor_packet->capacity.bytes.low_byte = uart_get_byte(9, CH_1);//);
		break;
	}
	uart_reset_receive(CH_1);//CONTROL_UART_1);
}
...
void update_leds()
{
	...
	uart_putchar(LEDS, CH_1);
	uart_putchar(leds, CH_1);
	uart_putchar(power_colour, CH_1);
	uart_putchar(power_intensity, CH_1);
}
...
void Roomba_LoadSong(uint8_t songNum, uint8_t* notes, uint8_t* notelengths, uint8_t numNotes)
{
	...
	uart_putchar(SONG, CH_1);
	uart_putchar(songNum, CH_1);
	uart_putchar(numNotes, CH_1);

	for (i=0; i<numNotes; i++)
	{
		uart_putchar(notes[i], CH_1);
		uart_putchar(notelengths[i], CH_1);
	}
}

void Roomba_PlaySong(int songNum)
{
	uart_putchar(PLAY, CH_1);
	uart_putchar(songNum, CH_1);
}
